...
#SBATCH --output=/groups/astro/dyson/Open/KerrLorenzMSF/GenerationCodes/Numerics-WorldTube/h1-radial-gen/logs/task_%a.txt
#SBATCH --error=/groups/astro/dyson/Open/KerrLorenzMSF/GenerationCodes/Numerics-WorldTube/h1-radial-gen/logs/task_%a.txt

PAIRS_FILE="/groups/astro/dyson/Open/KerrLorenzMSF/GenerationCodes/Numerics-WorldTube/h1-radial-gen/logs/pairs.txt"
SCRIPT_DIR="/groups/astro/dyson/Open/KerrLorenzMSF/GenerationCodes/Numerics-WorldTube/h1-radial-gen"
SAVEPATH="/lustre/hpc/astro/dyson/KerrCircMSFMaarten/h1-radial-gen/data"
...
DONE_FLAG="${SAVEPATH}/.done_a${a}_r${r0}"
if [ -f "$DONE_FLAG" ]; then
  echo "Task $SLURM_ARRAY_TASK_ID: output already exists, skipping."
  exit 0
fi
...
# Mark complete only on clean exit so requeued jobs don't skip partial output
