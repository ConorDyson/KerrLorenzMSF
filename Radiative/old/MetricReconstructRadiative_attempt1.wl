...
{
"SpinWeightedSpheroidalHarmonics`","Teukolsky`"
}]
Print["Refactor version 25-2-2026"]
...
NGrid[set_,rval_?NumericQ]:=Module[{kk},Table[Keys[set[[kk]]]->(Values[set[[kk]]]/.{r->rval}),{kk,1,Length[set]}]];
...
MetricReconstructRadiative[iConfig_,primarypath_]:=Module[{sallL,sallR,s2L,s2R,s1L,s1R,s0L,s0R,directory},
...
Print["Now define the S matrices"];
...
Print["Calculate fluxes and make a comparison with BHP Toolkit"];
If[a\[Omega]0==0,
...
(*Teuksolsky-Starobinskii identities *)
...
Join[comp,{Table[0,{Length@comp}]}]
];

GetComp[1][rtry_]:=Module[{comp},
Join[comp=Calcs1components[rtry],{Table[0,{Length@comp}]}]
];
